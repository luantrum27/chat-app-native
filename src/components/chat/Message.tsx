...
function Message({ message, isMyMessage }: IProps) {
    return (
        <View style={[{ display: 'flex', flexDirection: "row", alignItems: 'center', gap: 15 }, isMyMessage && { justifyContent: 'flex-start', flexDirection: 'row-reverse' }]}>
            ...
            <View>
                <View style={[{ display: 'flex', flexDirection: "row", alignItems: 'center' }, isMyMessage && { flexDirection: 'row-reverse' }]}>
                    <View style={{ maxWidth: '280px', paddingHorizontal: '16px', paddingVertical: '20px', backgroundColor: '#7269EF', borderRadius: 8, position: 'relative' }}>
                        <Text style={{ color: '#fff' }}>{message}</Text>
                    </View>
                    <Entypo name="dots-three-vertical" size={18} color={'#333'} />
                </View>
                <View style={[{ display: 'flex', flexDirection: "row", alignItems: 'center', gap: 2, marginTop: '5px' }, isMyMessage && {}]}>
